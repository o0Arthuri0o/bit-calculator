# Битовый калькулятор (Bit Calculator)

## Что это такое

Битовый калькулятор — это веб-приложение для выполнения битовых операций, операций сдвига и арифметических операций в стиле ARM. Приложение поддерживает работу с 32-битными знаковыми целыми числами и отображает результаты в десятичной, двоичной и шестнадцатеричной системах счисления.

### Основные возможности:

- **Логические операции**: AND, ORR (OR), EOR (XOR), BIC (Bit Clear), MVN (Move Not)
- **Операции сдвига**: LSL (Logical Shift Left), LSR (Logical Shift Right), ASR (Arithmetic Shift Right), ROR (Rotate Right)
- **Арифметические операции**: ADD, SUB, RSB (Reverse Subtract), MUL, CMP (Compare)
- **Визуализация**: Отображение битов в виде сетки с подсветкой изменений
- **Флаги состояния**: Отображение флагов N (Negative), Z (Zero), C (Carry), V (Overflow)
- **История операций**: Сохранение истории выполненных операций
- **PWA поддержка**: Возможность установки как Progressive Web App для работы оффлайн
...
2. Выберите режим ввода: **Десятичный** или **Двоичный**
3. Введите значения в поля r0 и r1 (для операций сдвига используйте поле shift)
4. Выберите нужную операцию из соответствующей вкладки:
   - **Логические операции** — для битовых операций
   - **Операции сдвига** — для сдвигов и ротаций
   - **Арифметические операции** — для математических операций
5. Нажмите кнопку операции для выполнения
6. Результат отобразится в поле результата с визуализацией битов

### Установка как приложение (PWA)

Приложение поддерживает установку как Progressive Web App (PWA), что позволяет использовать его оффлайн и как нативное приложение.

#### Установка на компьютере:

1. Откройте приложение в поддерживаемом браузере (Chrome, Edge, Firefox)
2. При первом посещении появится модальное окно с предложением установить приложение
3. Нажмите кнопку **"Установить"** в модальном окне
4. Или используйте кнопку установки в адресной строке браузера (иконка установки)
5. Приложение будет установлено и появится в списке установленных приложений

#### Установка на мобильных устройствах:

**Android (Chrome):**
1. Откройте приложение в браузере Chrome
2. Нажмите на меню браузера (три точки)
...
После установки приложение будет доступно как обычное приложение и сможет работать оффлайн.
...
- Приложение работает полностью оффлайн после установки как PWA
